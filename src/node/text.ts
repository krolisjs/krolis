...
import { data, hasRegister, waitIfNotRegister } from '../style/font';
...
import { JKeyFrameRich, RichAnimation } from '../animation/rich-animation';
import { Options } from '../animation/abstract-animation';
import { ceilBbox } from '../math/bbox';
import { CONTENT_CHANGE } from './text-event';
  ...
  override calBbox() {
    ...
    lineBoxList.forEach(lineBox => {
      lineBox.list.forEach(textBox => {
        ...
        if (hasRegister(fontFamily)) {
          const fontData = data[fontFamily];
